...
servidor.put('/usuarios/:id', async (request, reply) => {
    ...
    const resultado = await sql.query('UPDATE usuario SET nome = $1, senha = $2 WHERE id = $3, email = $4' [body.nome, body.senha, id, email])      
    reply.status(201).send({message: 'usuario alterado'})            
})
